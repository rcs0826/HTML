define(['index',
    ...
    function serviceOrdemCompra($timeout, $totvsresource, $rootScope, $filter, zoomService, dtsUtils, serviceLegend, $q){
        ...
        service.setConfiguration('mcc.ordem-compra-quotation.zoom');
        service.propertyFields = [
            ...
            {label: $rootScope.i18n('l-emission-date', [], 'dts/mcc'), property: 'ordem-compra.data-emissao',type: 'daterange'},  
            {label: $rootScope.i18n('l-delivery-date', [], 'dts/mcc'), property: 'prazo-compra.data-entrega',type: 'daterange'},  
        ...
        *******************************/	
        function parseDisclaimers(parameters) {
                    ...
                    if(parameters.init.filter) {
                        for (var property in parameters.init.filter) {                           
                            if (property == "ordem-compra.situacao") {
                                var field = {};
                                var situacao = service.propertyFields[6];
                                ...
                                cDescricao = service.propertyFields[6].propertyList.find( fproperty => fproperty.value == field.cStartValue); 
                                if  (service.propertyFields[6].value){
                                    service.propertyFields[6].value = {value: field.cStartValue, name: cDescricao.name}; 
                                }
                                else {
            ...
            if((fieldSituacao.cStartValue != undefined || fieldSituacao.cEndValue != undefined) && (fieldSituacao.cStartValue != 99)) cFields.push(fieldSituacao);                       
			/***********************************************************************************************************************/
    ...
    index.register.service('mcc.ordem-compra-quotation.zoom', serviceOrdemCompra);
});
