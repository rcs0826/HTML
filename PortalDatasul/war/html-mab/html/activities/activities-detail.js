define(['index'], function(index) {
	...
	function activitiesDetailCtrl($rootScope, 
							   $scope, 
							   $stateParams, 
							   $state,
							   appViewService,
		...
		*/
	        ...
	        $rootScope.$broadcast(TOTVSEvent.showQuestion, {
	            ...
	            callback: function(isPositiveResult) { // função de retorno
	                if (isPositiveResult) { // se foi clicado o botão confirmar
	                    ...
	                    bofr001Service.deleteRecord(controller.model ['cod-ativid'], function(result) {
	                    	if (!result.$hasError) {
	                            ...
	                            $state.go('dts/mab/activities.start');
	                        }
	                    });
	                }
	            }
	    ...
	    this.load = function(id) {
	        ...
	        bofr001Service.getRecord(id, function(record) {
	    ...
	    this.init = function() {
	        ...
	        if (appViewService.startView($rootScope.i18n('l-activities'), 'mab.activities.DetailCtrl', controller)) {             
	...
	index.register.controller('mab.activities.DetailCtrl', activitiesDetailCtrl);
