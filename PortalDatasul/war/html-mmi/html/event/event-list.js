define(['index',
	...
	function eventListCtrl($rootScope,
					  	   $scope,
						   $modal,
						   appViewService,
						   bomn259Service,
	    ...
	    this.openAdvancedSearch = function() {
	        ...
	        var modalInstance = $modal.open({
	          templateUrl: '/dts/mmi/html/event/event.advanced.search.html',
	          controller: 'mmi.event.SearchCtrl as controller',
	          size: 'lg',
	          resolve: {
	            model: function () {
	    ...
	    this.addDisclaimers = function() {
	        ...
	        if (controller.advancedSearch.codini || controller.advancedSearch.codfin) {
	            ...
	            // adicionar o disclaimer para o campo cod-evento, para faixa o value é "inicio;final"
	            controller.addDisclaimer('cod-evento', faixa, $rootScope.i18n('l-code') + deate);
	        }
	        // adicionar o disclaimer para descricao
	        if (controller.advancedSearch.descricao)
	            controller.addDisclaimer('des-evento', '*' + controller.advancedSearch.descricao  + '*', $rootScope.i18n('l-description') + ": " + controller.advancedSearch.descricao);	        	        
	    }
	    ...
	    this.removeDisclaimer = function(disclaimer) {
	        ...
	        if (disclaimer.property == null)
	            controller.quickSearchText = '';
	        if (disclaimer.property == 'cod-evento') {
	            controller.advancedSearch.codini = '';
	            controller.advancedSearch.codfin = '';
	        }  
	        if (disclaimer.property == 'des-evento') {
		...
		this.loadData = function(isMore) {
	        ...
	        if (controller.quickSearchText) {
	            where = "cod-evento matches '*" + this.quickSearchText + "*'";
	            where = where + " OR ";
	            where = where + "des-evento matches '*" + this.quickSearchText + "*'";
	        } else {
	        ...
	        // chama o findRecords
	        bomn259Service.findRecords(startAt, undefined, parameters, function(result) {	        
	            // se tem result
	            if (result) {
	        ...
	        $rootScope.$broadcast(TOTVSEvent.showQuestion, {
	            ...
	            callback: function(isPositiveResult) { // função de retorno
	                if (isPositiveResult) { // se foi clicado o botão confirmar
	                    ...
	                    bomn259Service.deleteRecord(record['cod-evento'], function(result) {
		...
		this.init = function() {
			...
			createTab = appViewService.startView($rootScope.i18n('l-event'), 'mmi.event.ListCtrl', controller);
			...
			if (previousView.controller) {			
				...
				if (createTab === false && previousView.controller !== "mmi.event.DetailCtrl" && previousView.controller !== "mmi.event.EditCtrl") {
	                return;
				}
			}
	...
	eventSearchCtrl.$inject = [ 	    
 		'$modalInstance', 
 	    'model'];
 	
 	function eventSearchCtrl ($modalInstance, 
 							  model) {
	...
	index.register.controller('mmi.event.ListCtrl', eventListCtrl);
	index.register.controller('mmi.event.SearchCtrl', eventSearchCtrl);
