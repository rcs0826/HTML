define(['index',
	...
	function subsystemEditCtrl($rootScope, 
	                			$scope, 
							    $stateParams, 
								$window, 
								$location,
	    ...
	    this.load = function(id) {
	        ...
	        bomn258Service.getRecord(id, function(result) {
	            // carrega no model
	            controller.model = result;
	            $("input[id^='des-sub-sist']").focus();
	        });
	    }
	    ...
	    this.save = function() {
	        ...
	        if ($state.is('dts/mmi/subsystem.edit')) {               
	            bomn258Service.updateRecord(this.model['cod-sub-sist'], this.model, function(result) {
	                // se gravou o registro com sucesso
	                controller.onSaveUpdate(true, result);
	            });
	        } else { // senão faz o create
	            bomn258Service.saveRecord(this.model, function(result) {
	                // se gravou o registro com sucesso
	    ...
	    this.isInvalidForm = function() {
	        ...
	        if (!this.model['cod-sub-sist'] || this.model['cod-sub-sist'].length === 0) {
	            isInvalidForm = true;
	            messages.push('l-subsystem');
	        }
	         
	        // verifica se a descrição foi informada
	        if (!this.model['des-sub-sist'] || this.model['des-sub-sist'].length === 0) {
	            isInvalidForm = true;
	    ...
	    this.redirectToDetail = function() {
	        $location.path('dts/mmi/subsystem/detail/' + this.model['cod-sub-sist']);
	    }
	    ...
	    this.init = function() {
	        ...
	        if (appViewService.startView($rootScope.i18n('l-subsystem'), 'mmi.subsystem.EditCtrl', controller)) {
	...
	index.register.controller('mmi.subsystem.EditCtrl', subsystemEditCtrl);
