define(['index',
	...
	function systemListCtrl($rootScope,
								 $scope,
								 $modal,
								 appViewService,
								 bomn257Service,
	    ...
	    this.openAdvancedSearch = function() {
	        ...
	        var modalInstance = $modal.open({
	          templateUrl: '/dts/mmi/html/system/system.advanced.search.html',
	          controller: 'mmi.system.SearchCtrl as controller',
	          size: 'lg',
	          resolve: {
	            model: function () {
	    ...
	    this.addDisclaimers = function() {
	        ...
	        if (controller.advancedSearch.codini || controller.advancedSearch.codfin) {
	            ...
	            controller.addDisclaimer('cod-sistema', faixa, $rootScope.i18n('l-code') + deate);
	        }
	        // adicionar o disclaimer para descricao
	        if (controller.advancedSearch.descricao)
	            controller.addDisclaimer('des-sistema', '*' + controller.advancedSearch.descricao  + '*', $rootScope.i18n('l-description') + ": " + controller.advancedSearch.descricao);	        	        
	    }
	    ...
	    this.removeDisclaimer = function(disclaimer) {
	        ...
	        if (disclaimer.property == 'des-sistema') {
		...
		this.loadData = function(isMore) {
	        ...
	        if (controller.quickSearchText) {
	            where = "cod-sistema matches '*" + this.quickSearchText + "*'";
	            where = where + " OR ";
	            where = where + "des-sistema matches '*" + this.quickSearchText + "*'";
	        } else {
	        ...
	        // chama o findRecords
	        bomn257Service.findRecords(startAt, undefined, parameters, function(result) {	        
	            // se tem result
	            if (result) {
	        ...
	        $rootScope.$broadcast(TOTVSEvent.showQuestion, {
	            ...
	            callback: function(isPositiveResult) { // função de retorno
	                if (isPositiveResult) { // se foi clicado o botão confirmar
	                    ...
	                    bomn257Service.deleteRecord(record['cod-sistema'], function(result) {
		...
		this.init = function() {
			createTab = appViewService.startView($rootScope.i18n('l-system'), 'mmi.system.ListCtrl', controller);			
			...
			if (previousView.controller) {			
				...
				if (createTab === false && previousView.controller !== "mmi.system.DetailCtrl" && previousView.controller !== "mmi.system.EditCtrl") {
	                return;
				}
			}
	...
	systemSearchCtrl.$inject = [ 	    
 		'$modalInstance', 
 	    'model'];
 	
 	function systemSearchCtrl ($modalInstance, 
 								model) {
	...
	index.register.controller('mmi.system.ListCtrl', systemListCtrl);
	index.register.controller('mmi.system.SearchCtrl', systemSearchCtrl);
