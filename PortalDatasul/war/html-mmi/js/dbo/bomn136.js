define(['index'], function(index) { 
	...
	// *** FACTORIES
	// *************************************************************************************
	bomn136Factory.$inject = ['$totvsresource'];	
	function bomn136Factory($totvsresource) {
		
		// definimos uma nova factory a partir do totvsresource, passando a URL do serviço de DBO
		var factory = $totvsresource.REST('/dts/datasul-rest/resources/dbo/mnbo/bomn136//:pNrOrdProdu//:pCdTarefa', {}, {});
        ...
        factory.updateRecord = function (id, model, callback) {
            return this.TOTVSUpdate({pNrOrdProdu: id.pNrOrdProdu, pCdTarefa: id.pCdTarefa}, model, callback);
        };
         
        factory.deleteRecord = function (id, callback) {
            return this.TOTVSRemove({pNrOrdProdu: id.pNrOrdProdu, pCdTarefa: id.pCdTarefa}, callback);
        };
		
		return factory;
	}
	
	index.register.factory('mmi.bomn136.Factory', bomn136Factory);
	
	// **************************************************************************************
	// *** SERVICE
	// **************************************************************************************
	
	bomn136Service.$inject = ['mmi.bomn136.Factory'];
	function bomn136Service(bomn136Factory) {
        ...
        this.findRecords = function (startAt, limitAt, parameters, callback) {
            ...
            return bomn136Factory.findRecords(parameters, callback);
        };
         
        this.getRecord = function (id, callback) {
            return bomn136Factory.getRecord(id, callback);
        };
         
        this.saveRecord = function (model, callback) {
            return bomn136Factory.saveRecord(model, callback);
        };
         
        this.updateRecord = function (id, model, callback) {
            return bomn136Factory.updateRecord(id, model, callback);
        };
         
        this.deleteRecord = function (id, callback) {
            return bomn136Factory.deleteRecord(id, callback);
        };
	}
	
	index.register.service('mmi.bomn136.Service', bomn136Service);
