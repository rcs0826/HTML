...
*/
/*///////////////////////////////////////////////////////
Alterações feitas pela TOTVS estão com comentário //TOTVS
///////////////////////////////////////////////////////*/
(function(){
    'use strict';
    angular.module('gantt', ['gantt.templates', 'angularMoment'])
        ...
        return {
            ...
            scope: {
                ...
                api: '=?',
                options: '=?',
                taskContextMenu: '=?', //TOTVS - menu de contexto na tarefa
                taskPopoverOptions: '=?', //TOTVS - popover com detalhes da tarefa
            },
...
angular.module('gantt.templates', []).run(['$templateCache', function ($templateCache) {
    $templateCache.put('template/gantt.tmpl.html',
        ...
        '        <div class="gantt-task" ng-class="task.model.classes" context-menu="$parent.gantt.options.value(\'taskContextMenu\')" model="task" uib-popover-template="($parent.gantt.options.value(\'taskPopoverOptions\')).taskPopoverTemplate"  popover-trigger="\'outsideClick\'" popover-placement="auto" popover-append-to-body="($parent.gantt.options.value(\'taskPopoverOptions\')).popoverAppendToBody">\n' + //TOTVS - classes context-menu e model adicionadas para o menu de contexto na tarefa
    ...
    angular.module('gantt.overlap', ['gantt', 'gantt.overlap.templates']).directive('ganttOverlap', ['moment', '$timeout', function(moment, $timeout) {
        return {
            ...
            link: function(scope, element, attrs, ganttCtrl) {
                ...
                //TOTVS
                function setRowHeight(row, overlapGroupMaxLength) {
                    
                    if (overlapGroupMaxLength > 0) {
                        row.model.height = rowHeight * overlapGroupMaxLength + "px";
                    } else {
                        row.model.height = rowHeight + "px";
                    }
                }

                //TOTVS
                function setTasksMargin(overlapGroups, overlapGroupMaxLength) {
                    
                    overlapGroups.forEach(function(overlapGroup) {
        
                        overlapGroup.tasks.forEach(function(task) {
                            if (task.$element) {
                                task.$element.css('margin-top', rowHeight * task.level + 2.797 + 'px');
                            }

                            if (task.level + 1 > overlapGroupMaxLength) { 
                                overlapGroupMaxLength = task.level + 1;
                            }
                        });

                    })

                    return overlapGroupMaxLength;
                }

                //TOTVS
                /* Função handleOverlaps original
                ...
                function handleOverlaps(tasks) {
                    ...
                    if (tasks.length > 0) {
                        ...
                        //TOTVS Lista de grupos de tarefas em conflito, cada objeto vai ter uma lista de tarefas
                        var overlapGroups = [];
                        
                        //TOTVS Grupo com a maior quantidade de tarefas, usado para definir a altura da linha
                        var overlapGroupMaxLength = 0;

                        //TOTVS Reseta a margem de todas as tarefas
                        overlapGroups.push({
                            tasks: tasks
                        });
                        setTasksMargin(overlapGroups, overlapGroupMaxLength);

                        //TOTVS Reseta a altura da linha
                        setRowHeight(tasks[0].row, 0);
                        ...
                        previousTask.range = previousRange;
                        previousTask.level = 0;
                        
                        var newOverlapGroup = previousRange;
                        newOverlapGroup.tasks = [previousTask];
                        overlapGroups.push(newOverlapGroup);
                        ...
                        for (var i = 1, l = tasks.length; i < l; i++) {
                            ...
                            handleOverlapConflict(overlapGroups, task);
                        ...
                        overlapGroupMaxLength = setTasksMargin(overlapGroups, overlapGroupMaxLength);
                        setRowHeight(tasks[0].row, overlapGroupMaxLength);
    
                        handleTaskNonOverlaps(newOverlapsTasks, tasks);
                    }
                }

                function handleOverlapConflict(overlapGroups, task) {
                    var taskOverlaps = false;
                    
                    for (var i = 0; i < overlapGroups.length; i++) {
                        const overlapGroup = overlapGroups[i];
                        
                        for (var j = 0; j < overlapGroup.tasks.length; j++) {
                            const overlapGroupTask = overlapGroup.tasks[j];
                            
                            if (task.range.overlaps(overlapGroupTask.range) && 
                                (task.level == overlapGroupTask.level)) {
                                taskOverlaps = true;
                                task.level = overlapGroup.tasks.length;
                                task.overlapGroup = overlapGroup;
                                overlapGroup.tasks.push(task);
                                break;
                            }
                        };
                    };

                    // Verifica se a tarefa atual sobrepoe uma tarefa em outro grupo
                    if (taskOverlaps) {
                        loop:
                        for (var i = 0; i < overlapGroups.length; i++) {
                            const overlapGroup = overlapGroups[i];
                            
                            if (overlapGroup != task.overlapGroup) {
                                for (var j = 0; j < overlapGroup.tasks.length; j++) {
                                    const overlapGroupTask = overlapGroup.tasks[j];
                    ...
                    if (!taskOverlaps) {
                        var newOverlapGroup = task.range;
                        newOverlapGroup.tasks = [task];
                        overlapGroups.push(newOverlapGroup);
                    }
                ...
                if (scope.enabled) {
                    ...
                    //TOTVS
                    api.rows.on.displayed(scope, function(rows){
                        // TODO: Mimicked functionality from api.data.on.change to defer until element creation, but not ideal.  Refactor necessary to raise 'displayed' event after task is fully drawn.
                        $timeout(function() {
                            for (var i = 0; i < rows.length; i++) {
                                const row = rows[i];
