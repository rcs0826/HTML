define(['index',    
    ...
    // *** CONTROLLER 
    // *************************************************************************************
    documentosExportAvisosCobReprocessarController.$inject = ['$rootScope','$scope', '$modalInstance', 
    '$timeout', '$state', 'TOTVSEvent', 'hrc.documentosExportAvisosCob.Factory','tmpDoctoAvisoCob','disclaimers','tmpNaoSelecDoctoAvisoCob'];
    function documentosExportAvisosCobReprocessarController($rootScope,$scope, $modalInstance,
        ...
        this.init = function () {               
            ...
            documentosExportAvisosCobFactory.comunicarAvisoComErro(_self.tmpDoctoAvisoCob,_self.disclaimers,false,_self.tmpNaoSelecDoctoAvisoCob, function (result) { 
                _self.hasDoneSearch = true;
                if (result) { 
                    _self.reprocessDoctList = result.tmpDocAvisoExcSit;                    
        ...
        $.extend(this, documentosExportAvisosCobReprocessarController);
    }

    index.register.controller('hrc.documentosExportAvisosCobReprocessar.Control', documentosExportAvisosCobReprocessarController);
});
