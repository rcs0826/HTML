...
var AcrEvent, AcrRestService, AcrURL, AcrUtil;
...
// Servicos
AcrRestService = '/dts/datasul-rest/resources/api/fch/fchacr/fchacr';
...
define(['index'], function (index) {
    ...
    */
    function booleanFormat($rootScope) {
		...
		return function (sentence, params) {
            ...
            if (sentence) {
                return $rootScope.i18n('l-yes', undefined, 'dts/acr');
            } else {
                return $rootScope.i18n('l-no', undefined, 'dts/acr');
            }
	    };
	}
    ...
    function dateFormat($rootScope, $filter) {
		...
		return function (sentence, params) {
            ...
            var filterDate = $filter('date'),
                dateF      = $rootScope.i18n('l-date-format', undefined, 'dts/acr');
    ...
    */
	...
	function serviceHelper($rootScope, $filter) {
        ...
        service.parseDateRangeToDisclaimer = function (range, property, label, dtsModule, fixed) {
            ...
            if (range.end) {
                disclaimer.title += startDateLabel          + ' ' +
                                    $rootScope.i18n('l-to', undefined, 'dts/acr') + ' ' +
                                    this.formatDate(range.end);
            } else {
                disclaimer.title += $rootScope.i18n('l-after-prep', undefined, 'dts/acr') + ' ' + startDateLabel;
            }
        ...
        */
        service.parseCharRangeToDisclaimer = function (range, property, label, dtsModule, fixed) {
            ...
            if (range.start !== "") {
                disclaimer.title += $rootScope.i18n('l-between', undefined, 'dts/acr') + ' "' +
                                    range.start                  + '" ' +
                                    $rootScope.i18n('l-and', undefined, 'dts/acr')     + ' "' +
                                    range.end                    + '"';
            } else {
                disclaimer.title += $rootScope.i18n('l-between', undefined, 'dts/acr') + ' " " ' +
                                    $rootScope.i18n('l-and', undefined, 'dts/acr')     + ' "'    +
        ...
        */
        service.parseTypeToDisclaimer = function (type, key, model, title, dtsModule, fixed, fields) {
            ...
            case 'select':
                ...
                //console.log('select', type, key, model, title, dtsModule, fixed, fields);
        ...
        */
        service.parseProfileDataArrayToList = function (profileDataArray, nameData, fieldId, fieldDesc) {
            ...
            MultipleSelectResult.prototype.toString = function () {
                return this.objSelected.length + ' - ' + $rootScope.i18n('l-selecteds', undefined, 'dts/acr');
            };
	...
	index.register.service('acr.utils.Service', serviceHelper);
