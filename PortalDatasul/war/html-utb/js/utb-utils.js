...
define(['index'], function (index) {
    ...
    */
    function booleanFormat($rootScope) {
		...
		return function (sentence, params) {
            ...
            if (sentence) {
                return $rootScope.i18n('l-yes', undefined, 'dts/utb');
            } else {
                return $rootScope.i18n('l-no', undefined, 'dts/utb');
            }
	    };
	}
    ...
    function dateFormat($rootScope, $filter) {
		...
		return function (sentence, params) {
            ...
            var filterDate = $filter('date'),
                dateF      = $rootScope.i18n('l-date-format', undefined, 'dts/utb');
    ...
    */
	...
	function serviceHelper($rootScope, $filter) {
        ...
        */
        service.formatDate = function (value) {
            ...
            var filterDate = $filter('date'),
                dateFormat = $rootScope.i18n('l-date-format', undefined, 'dts/utb');
        ...
        service.parseDateRangeToDisclaimer = function (range, property, label, dtsModule, fixed) {
            ...
            if (range.end) {
                disclaimer.title += startDateLabel          + ' ' +
                                    $rootScope.i18n('l-to', undefined, 'dts/utb') + ' ' +
                                    this.formatDate(range.end);
            } else {
                disclaimer.title += $rootScope.i18n('l-after-prep', undefined, 'dts/utb') + ' ' + startDateLabel;
            }
        ...
        */
        service.parseCharRangeToDisclaimer = function (range, property, label, dtsModule, fixed) {
            ...
            if (range.start !== "") {
                disclaimer.title += $rootScope.i18n('l-between', undefined, 'dts/utb') + ' "' +
                                    range.start                  + '" ' +
                                    $rootScope.i18n('l-and', undefined, 'dts/utb')     + ' "' +
                                    range.end                    + '"';
            } else {
                disclaimer.title += $rootScope.i18n('l-between', undefined, 'dts/utb') + ' " " ' +
                                    $rootScope.i18n('l-and', undefined, 'dts/utb')     + ' "'    +
	...
	index.register.service('utb.utils.Service', serviceHelper);
