...
define(['index'], function (index) {
	...
	searchQuotationItemLastShoppingcontroller.$inject = [
	...
	function searchQuotationItemLastShoppingcontroller(
		fchdis0063,
		customService,
		$scope,
		$timeout,
		...
		lastShoppingController.quotationController = $scope.quotationController;

		lastShoppingControllerquotation = $scope.quotationController.order;

		lastShoppingController.nrPedido = $scope.quotationController.order['nr-pedido'];

		lastShoppingController.isRepresentative = $scope.quotationController.isRepresentative;

		lastShoppingController.isCustomer = $scope.quotationController.isCustomer
		lastShoppingController.pesquisaVisibleFields = $scope.quotationController.pesquisaVisibleFields;
		...
		this.getPeriodShopping = function () {
			...
			userPreference.getPreference('mpd.portal.showitimg')]).then(function (results) {
				...
				if (results && results[0].prefValue) {
					lastShoppingController.shoppingPeriod = results[0].prefValue;
				}
		...
		lastShoppingController.addItens = function (item) {
			...
			if (item['qt-un-fat'] <= 0) {
				$rootScope.$broadcast(TOTVSEvent.showNotification, [{ type: 'warning', title: 'Itens', detail: 'Informe quantidade no item para adiciona-lo ao pedido.' }]);
		...
		lastShoppingController.addItensWithTimeOut = function (item) {
			...
			if (item['qt-un-fat'] > 0) {
				...
				item.itemsToAfterDrawAttention = false;
				$rootScope.$broadcast(TOTVSEvent.showNotification, [{ type: 'warning', title: $rootScope.i18n('l-items-qtd'), detail: $rootScope.i18n('l-enter-quantity-2') }]);
			}
		}
		...
		lastShoppingController.reloadItems = function (gotoitems) {
			...
			if (!gotoitems) {
				...
				$rootScope.$broadcast(TOTVSEvent.showNotification, [{ type: 'info', title: $rootScope.i18n('l-items-qtd'), detail: $rootScope.i18n('l-added-to-quotation') }]);
			}
		};
			...
			fchdis0063.searchproducts(
				...
				, function (data) {
					...
					if (data.dsOrderItemSearch.length == 0) {
						$rootScope.$broadcast(TOTVSEvent.showNotification, [{ type: 'warning', title: $rootScope.i18n('l-search-with-results'), detail: $rootScope.i18n('l-another-search-term') }]);
					}
	...
	index.register.controller('salesorder.quotation.searchItemLastShopping.Controller', searchQuotationItemLastShoppingcontroller);
