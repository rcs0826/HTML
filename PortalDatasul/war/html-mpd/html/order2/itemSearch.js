...
define(['index'], function(index) {
		...
		function searchItemcontroller(
			...
			$scope.$on("salesorder.portal.editableprice", function (event,data) {
				searchController.editablePrice = $scope.orderController.editablePrice;
			});
			...
			searchController.nrPedido = $scope.orderController.order['nr-pedido'];

			searchController.isRepresentative = $scope.orderController.isRepresentative;
			searchController.isCustomer = $scope.orderController.isCustomer
			searchController.pesquisaVisibleFields = $scope.orderController.pesquisaVisibleFields;
			searchController.editablePrice = $scope.orderController.editablePrice;
			...
			fchdis0035.getSalesOrderPreferences(function (data) {
				
				if (data) {
					for (var i = 0; i < data.length; i++) {
						if (data[i].prefId == 'salesorder.portal.SoItemCli') {
							searchController.searchParams.SoItemCli = data[i].valor == 'true';
						} else if (data[i].prefId == 'salesorder.portal.ConsideraFamilia') {
							searchController.searchParams.ConsideraFamilia  = data[i].valor == 'true';
						} else if (data[i].prefId == 'salesorder.portal.FamilyBusiness') {
							searchController.searchParams.FamilyBusiness      = data[i].valor;
						} else if (data[i].prefId == 'salesorder.portal.GroupStocks') {
							searchController.searchParams.GroupStocks          = data[i].valor;
						} else if (data[i].prefId == 'salesorder.portal.ConsideraGrupo') {
							searchController.searchParams.ConsideraGrupo    = data[i].valor == 'true';
						} else if (data[i].prefId == 'salesorder.portal.SoItemTabPrecoPedido') {
							searchController.searchParams.SoItemTabPrecoPedido = data[i].valor == 'true';
						} else if (data[i].prefId == 'salesorder.portal.simpleSearchType') {
							searchController.simpleSearchType  = data[i].valor;
						} else if (data[i].prefId == 'salesorder.portal.NrTabPre') {
							searchController.searchParams.NrTabPre = data[i].valor;
						}  
					}
        ...
        searchController.loadMore = function() {
			...
			searchController.consigSearchItens = function() {
				...
				var modalInstance = $modal.open({
					templateUrl: '/dts/mpd/html/order2/itemSearch.params.html',
					controller: 'salesorder.order2.searchItemParam.Controller as searchItemParamController',
					size: 'lg',
					resolve: {
		...
		index.register.controller('salesorder.order2.searchItems.Controller', searchItemcontroller);
		...
		index.register.controller('salesorder.order2.searchItemParam.Controller', searchItemParamController);
