...
define(['index'], function(index) {
	...
	function orderitemsgridportalcontroller(
		...
		$scope.$on("salesorder.portal.loadorder", function (event,data) {
			...
			if (data != "orderItems") {
				...
				itemsGridController.orderId = $scope.orderController.order['nr-pedido']
				itemsGridController.orderItens = $scope.orderController.orderItens;
				itemsGridController.visibleFields = $scope.orderController.pedItemListaVisibleFields;
				itemsGridController.orderDisabled = $scope.orderController.orderDisabled;
				itemsGridController.editablePrice = $scope.orderController.editablePrice;
				itemsGridController.itemCadastroVisibleFields = $scope.orderController.itemCadastroVisibleFields;
				...
				userPreference.getPreference('mpd.portal2.showChild').then(function(results) {
		...
		itemsGridController.itemsGridEdit = function (event, column) {
			...
			if (itemsGridController.inlineEditActive) {
				...
				if (column.column == "observacao") return;
		...
		itemsGridController.openItemModal = function() {
			...
			var modalInstance = $modal.open({
			  templateUrl: '/dts/mpd/html/order2/orderItem.html',
			  controller: 'salesorder.order2.orderItem.Controller as itemController',
			  size: 'lg',
			  backdrop: 'static',
			  resolve: {
				modalParams: function() {
					return {
					   ...
					   ttParamBonif: $scope.orderController.ttParamBonif
					};
				}
			  }
			});
		...
		itemsGridController.removeOrderItem = function() {
			...
			for (var i = 0; i < selectitems.length; i++) {
			   ...
			   messageUtils.question( {
				title: $rootScope.i18n('l-remove-item'),
		...
		itemsGridController.setUserPreferences = function() {
			$q.all([userPreference.setPreference('mpd.portal2.showChild', itemsGridController.showChild)]).then(function() {});
		}
	...
	function modalItemConfigNarrative($modal) {
		...
		this.open = function (params) {
			var instance = $modal.open({
				templateUrl: '/dts/mpd/html/order2/itemconfig.narrative.html',
				controller: 'order.itemconfig.narrative.Controller as controller',
				backdrop: 'static',
				keyboard: false,
				size: 'lg',
				resolve: {
	...
	index.register.controller('salesorder.order2.orderitems.Controller', orderitemsgridportalcontroller);
	index.register.service('order.itemconfig.narrative.modal', modalItemConfigNarrative);
	index.register.controller('order.itemconfig.narrative.Controller', itemConfigNarrativeModalController);
