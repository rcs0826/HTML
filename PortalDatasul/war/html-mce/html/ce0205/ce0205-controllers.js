    define([  
       'index',
        ...
        '/dts/mce/js/api/fch/fchmat/fchmatmaterialrequisition-services.js',
        '/dts/mce/js/api/fch/fchmat/fchmatintegrationaccountcostcenter-services.js'       
], function (index) {

        ce0205EditCtrl.$inject = ['$rootScope', '$scope', '$location', 'totvs.app-main-view.Service', 'mce.fchmatMaterialRequisitionFactory.factory', 'mce.fchmatIntegrationAccountCostCenterFactory.factory', 'mce.utils.Service', 'TOTVSEvent', '$stateParams'];

        function ce0205EditCtrl($rootScope, $scope, $location, appViewService, fchmatMaterialRequisitionFactory, fchmatIntegrationAccountCostCenterFactory, mceUtils, TOTVSEvent, $stateParams) {
            ...
            */
            this.getDefaults = function (fieldName) {
                ...
                if (controller.ttManualRequisition[fieldName] != undefined) {
                    ...
                    fchmatMaterialRequisitionFactory.setDefaultsRequisition({
                            pType: "",
            ...
            */
            this.changeAccount = function () {
                ...
                if (controller.ttManualRequisition['ct-codigo'] !== undefined &&
                    ...
                    //retorna se habilita o nao o campo centro de custo
            ...
            */
            ...
            this.sucessNotification = function () {
                ...
                $rootScope.$broadcast(TOTVSEvent.showNotification, {
                    type: 'success',
                    title: $rootScope.i18n('l-manual-material-requisition'),
                    detail: $rootScope.i18n('l-success-action', [$rootScope.i18n('l-manual-material-requisition')], 'dts/mce')
                });
            };

            /* Função....: validateAndCreateRequisition
               Descrição.: disparada ao clicar no botão Salvar
               Parâmetros: <não há> 
            */
            this.validateAndCreateRequisition = function () {
                ...
                fchmatMaterialRequisitionFactory.validateLot({}, controller.ttManualRequisition, function (result) {
                    if (result.pValidate) { //Data de Validade do Lote Vencida
                        $rootScope.$broadcast(TOTVSEvent.showQuestion, {
                            ...
                            callback: function (isPositiveResult) {
                                if (isPositiveResult) {
                                    // Cria requisicao
                                    fchmatMaterialRequisitionFactory.createRequisition({}, controller.ttManualRequisition,
                                        function (result) {
                                            if (!result.$hasError) {
                                                controller.sucessNotification();
                                                controller.emptyModels(); // limpa objetos de tela
                                                controller.init(isSaveNew);
                                                $('#estab').focus();
            ...
            */
            this.isInvalidForm = function () {
                ...
                missingFields = mceUtils.validateMissingFields($("#requisitionFormFields"), false);
            ...
            this.init = function (isSaveNew) {
                ...
                createTab = appViewService.startView($rootScope.i18n('l-manual-material-requisition'), 'mce.ce0205.EditCtrl', controller);

                if (createTab === false && !isSaveNew && !isStateParams) {
                    return;
                } else {                    
                    $('#estab').focus();
                ...
                if (isStateParams) {
                    ...
                    fchmatMaterialRequisitionFactory.returnDataFromDashboard({}, 
                    ...
                    fchmatMaterialRequisitionFactory.initializeInterface(function (result) {
                        if (!result.$hasError) {
            ...
            */
            ...
            /* Função....: fillBalanceFields
               Descrição.: Carrega campos retornados do botao Selecionar Saldo
               Parâmetros: objSelected: Objeto retornado
            */
            this.fillBalanceFields = function (objSelected, oldValueSelected) {
                controller.ttManualRequisition['cod-depos']    = objSelected['cod-depos'];
                controller.ttManualRequisition['cod-localiz']  = objSelected['cod-localiz'];
                controller.ttManualRequisition['dt-vali-lote'] = objSelected['dt-vali-lote'];
                controller.ttManualRequisition['cod-refer']    = objSelected['cod-refer'];            
                controller.ttManualRequisition['lote']         = objSelected['lote'];
            }
        ...
        /*   REGISTERS                                                                    */
        /**********************************************************************************/
        index.register.controller('mce.ce0205.EditCtrl', ce0205EditCtrl);
