...
define([
	...
	factory = function ($totvsresource, $cacheFactory, factoryGeneric, factoryGenericCreateUpdate,
		...
		var cache = $cacheFactory('crm.ticket-flow-status-recur.Cache'),
			factory = $totvsresource.REST(CRMRestService + '1105/:method/:id/:resource', undefined,
		...
		factory.getResourcesByFlowStatus = function (statusId, callback, isAllowedCache, notTakeTheCache) {

			isAllowedCache = isAllowedCache !== false;

			var idCache = 'resourcesbystatus' + statusId.toString(),
				result = (isAllowedCache && !notTakeTheCache ? cache.get(idCache) : undefined);

			if (result) {
				if (callback) { callback(result); }
			} else {
				return this.TOTVSQuery({ method: 'resourcesbyflowstatus', statusid: statusId }, function (data) {
	...
	index.register.factory('crm.crm_ocor_fluxo_status_recur.factory', factory);
