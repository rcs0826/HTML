...
define([
	...
	'css!/dts/crm/assets/css/crm.css',
	...
	factoryGenericZoom = function () {
		return {
			zoom : function (parameters, options, callback, instance) {
				options = options || {};
				options.type = options.type || 3;
				if (!options.end) { options.end = 25; }
	...
	factoryGenericDetail = function () {
		return {
			getRecord: function (id, callback) {
				return this.TOTVSQuery({id: id, method: 'detail'}, function (result) {
					if (callback) {
						callback((result && result.length > 0 ? result[0] : undefined));
					}
				});
	...
	index.register.factory('crm.generic.factory',                   factoryGeneric);
	index.register.factory('crm.generic.zoom.factory',              factoryGenericZoom);
	index.register.factory('crm.generic.typeahead.factory',         factoryGenericTypeahead);
	index.register.factory('crm.generic.create.update.factory',     factoryGenericCreateUpdate);
	index.register.factory('crm.generic.delete.factory',            factoryGenericDelete);
	index.register.factory('crm.generic.detail.factory',            factoryGenericDetail);
