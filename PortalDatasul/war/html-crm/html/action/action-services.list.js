...
define([
	...
	controller = function ($scope, $rootScope, TOTVSEvent, helper, factory, modalEdit, userPreferenceModal) {
		...
		this.remove = function (item) {
			...
			$rootScope.$broadcast(TOTVSEvent.showQuestion, {
				...
				text: $rootScope.i18n('msg-confirm-delete', [
					$rootScope.i18n('nav-action', [], 'dts/crm').toLowerCase(), item.nom_acao
				], 'dts/crm'),
				callback: function (isPositiveResult) {
					...
					factory.deleteRecord(item.num_id, function (result) {
						...
						$rootScope.$broadcast(TOTVSEvent.showNotification, {
							type: 'success',
							title: $rootScope.i18n('nav-action', [], 'dts/crm'),
		...
		this.userSettings = function () {
			userPreferenceModal.open({ context: 'action.list' });
		};
		...
		this.init = function init() {
			...
			var viewName = $rootScope.i18n('nav-action', [], 'dts/crm'),
				viewController = 'crm.action.list.control';
	...
	controller.$inject = [
		'$scope', '$rootScope', 'TOTVSEvent', 'crm.helper', 'crm.crm_acao.factory',
		'crm.action.modal.edit', 'crm.user.modal.preference'
	];
	...
	index.register.controller('crm.action.list.control', controller);
});
