...
define([
	...
	controller = function ($scope, $rootScope, TOTVSEvent, helper, factory, modalEdit,
							modalAdvancedSearch, userPreferenceModal) {
		...
		this.remove = function (item) {
			...
			$rootScope.$broadcast(TOTVSEvent.showQuestion, {
				...
				text: $rootScope.i18n('msg-confirm-delete', [
					$rootScope.i18n('nav-strategy', [], 'dts/crm').toLowerCase(), item.des_estrateg_vda
				], 'dts/crm'),
		...
		this.userSettings = function () {
			userPreferenceModal.open({ context: 'strategy.list' });
		};
		...
		this.init = function init() {
			...
			var viewName = $rootScope.i18n('nav-strategy', [], 'dts/crm'),
				viewController = 'crm.strategy.list.control';
	...
	controller.$inject = [
		'$scope', '$rootScope', 'TOTVSEvent', 'crm.helper', 'crm.crm_estrateg_vda.factory',
		'crm.strategy.modal.edit', 'crm.strategy.modal.advanced.search', 'crm.user.modal.preference'
	];
	...
	index.register.controller('crm.strategy.list.control', controller);
});
