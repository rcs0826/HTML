...
define([
	...
	controller = function ($scope, $rootScope, TOTVSEvent, helper, factory, modalEdit, userPreferenceModal) {
		...
		this.remove = function (item) {
			...
			$rootScope.$broadcast(TOTVSEvent.showQuestion, {
				...
				text: $rootScope.i18n('msg-confirm-delete', [
					$rootScope.i18n('nav-phase', [], 'dts/crm').toLowerCase(), item.des_fase
				], 'dts/crm'),
				callback: function (isPositiveResult) {
					...
					factory.deleteRecord(item.num_id, function (result) {
						...
						$rootScope.$broadcast(TOTVSEvent.showNotification, {
							type: 'success',
							title: $rootScope.i18n('nav-phase', [], 'dts/crm'),
		...
		this.userSettings = function () {
			userPreferenceModal.open({ context: 'phase.list' });
		};
		...
		this.init = function init() {
			...
			var viewName = $rootScope.i18n('nav-phase', [], 'dts/crm'),
				viewController = 'crm.phase.list.control';
	...
	controller.$inject = [
		'$scope', '$rootScope', 'TOTVSEvent', 'crm.helper', 'crm.crm_fase_estrateg_vda.factory',
		'crm.phase.modal.edit', 'crm.user.modal.preference'
	];
	...
	index.register.controller('crm.phase.list.control', controller);
});
