...
define([
	...
	modal = function ($modal) {
		this.open = function (params) {
			var instance = $modal.open({
				templateUrl: '/dts/crm/html/phase/phase.edit.html',
				controller: 'crm.phase.edit.control as controller',
				backdrop: 'static',
				keyboard: false,
	...
	controller = function ($rootScope, $scope, $modalInstance, parameters, TOTVSEvent,
		...
		this.save = function (isToContinue) {
			...
			fnAfterSave = function (item) {
				...
				$rootScope.$broadcast(TOTVSEvent.showNotification, {
					type: 'success',
					title: $rootScope.i18n('l-phase', [], 'dts/crm'),
		...
		this.isInvalidForm = function () {
			...
			if (!model.des_fase || model.des_fase.trim().length === 0) {
				isInvalidForm = true;
				messages.push('l-name');
			}

			if (isInvalidForm) {
				helper.showInvalidFormMessage('l-phase', messages);
			}
		...
		this.convertToSave = function () {
			...
			vo.des_fase = model.des_fase;

			vo.log_livre_1 = model.log_livre_1;
			vo.log_fechto_fase = model.log_fechto_fase;
			vo.log_fase_perdido = model.log_fase_perdido;
	...
	controller.$inject = [
		...
		'crm.crm_fase_estrateg_vda.factory'
	];
	...
	index.register.service('crm.phase.modal.edit', modal);
	index.register.controller('crm.phase.edit.control', controller);
});
