...
define([
	...
	modal = function ($modal) {
		this.open = function (params) {
			var instance = $modal.open({
				templateUrl: '/dts/crm/html/attachment-type/attachment-type.edit.html',
				controller: 'crm.attachment-type.edit.control as controller',
				backdrop: 'static',
				keyboard: false,
				size: 'lg',
	...
	controller = function ($rootScope, $scope, $modalInstance, parameters, TOTVSEvent,
		...
		this.save = function (isToContinue) {
			...
			fnAfterSave = function (item) {
				...
				$rootScope.$broadcast(TOTVSEvent.showNotification, {
					type: 'success',
					title: $rootScope.i18n('l-attachment-type', [], 'dts/crm'),
					detail: $rootScope.i18n(message, [
						$rootScope.i18n('l-attachment-type', [], 'dts/crm'),
						item.nom_tip_anexo
					], 'dts/crm')
				});
		...
		this.isInvalidForm = function () {
			...
			if (!model.nom_tip_anexo || model.nom_tip_anexo.trim().length === 0) {
				isInvalidForm = true;
				messages.push('l-name');
			}

			if (isInvalidForm) {
				helper.showInvalidFormMessage('l-attachment-type', messages);
			}
		...
		this.convertToSave = function () {
			...
			vo.nom_tip_anexo = model.nom_tip_anexo;
		...
		this.model = parameters.attachmentType ? angular.copy(parameters.attachmentType) : {};
	...
	controller.$inject = [
		...
		'crm.legend', 'crm.crm_tip_anexo.factory'
	];
	...
	index.register.service('crm.attachment-type.modal.edit', modal);
	index.register.controller('crm.attachment-type.edit.control', controller);
});
