...
define([
	...
	controller = function ($scope, $rootScope, TOTVSEvent, helper, factory, modalEdit, userPreferenceModal) {
				...
				callback: function (isPositiveResult) {
					...
					factory.deleteRecord(item.num_id, function (result) {
						...
						$rootScope.$broadcast(TOTVSEvent.showNotification, {
							type: 'success',
							title: $rootScope.i18n('nav-result', [], 'dts/crm'),
		...
		this.userSettings = function () {
			userPreferenceModal.open({ context: 'result.list' });
		};
		...
		this.init = function init() {
			...
			var viewName = $rootScope.i18n('nav-result', [], 'dts/crm'),
				viewController = 'crm.result.list.control';
	...
	controller.$inject = [
		'$scope', '$rootScope', 'TOTVSEvent', 'crm.helper', 'crm.crm_restdo.factory',
		'crm.result.modal.edit', 'crm.user.modal.preference'
	];
	...
	index.register.controller('crm.result.list.control', controller);
});
