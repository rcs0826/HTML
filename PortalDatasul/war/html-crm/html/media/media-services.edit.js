...
define([
	...
	modal = function ($modal) {
		this.open = function (params) {
			var instance = $modal.open({
				templateUrl: '/dts/crm/html/media/media.edit.html',
				controller: 'crm.media.edit.control as controller',
				backdrop: 'static',
				keyboard: false,
	...
	controller = function ($rootScope, $scope, $modalInstance, parameters, TOTVSEvent,
		...
		this.save = function (isToContinue) {
			...
			fnAfterSave = function (item) {
				...
				$rootScope.$broadcast(TOTVSEvent.showNotification, {
					type: 'success',
					title: $rootScope.i18n('l-media', [], 'dts/crm'),
					detail: $rootScope.i18n(message, [
						$rootScope.i18n('l-media', [], 'dts/crm'),
						item.nom_mid_relacto
					], 'dts/crm')
				});
		...
		this.isInvalidForm = function () {
			...
			if (!model.nom_mid_relacto || model.nom_mid_relacto.trim().length === 0) {
				isInvalidForm = true;
				messages.push('l-name');
			}

			if (isInvalidForm) {
				helper.showInvalidFormMessage('l-media', messages);
			}
		...
		this.convertToSave = function () {
			...
			vo.nom_mid_relacto = model.nom_mid_relacto;
		...
		this.model = parameters.media ? angular.copy(parameters.media) : {};
	...
	index.register.service('crm.media.modal.edit', modal);
	index.register.controller('crm.media.edit.control', controller);
});
