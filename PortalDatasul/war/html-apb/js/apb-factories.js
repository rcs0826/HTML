...
define([
	...
	apbUserDataFactory.$inject = ["$http", "$rootScope"];
	function apbUserDataFactory($http, $rootScope) {

		$http.get(
			'/dts/datasul-rest/resources/api/fch/fchapb/userpreference/preference/menu.defaultPage'
		...
		if ($rootScope.currentuser && $rootScope.currentuser.then) {
			$rootScope.currentuser.then(function () {
				...
				if ($rootScope.currentuser && $rootScope.currentuser.hasOwnProperty("user-entity"))
					return $rootScope.currentuser;
				$http.get("/dts/datasul-rest/resources/api/fch/fchapb/userpreference").then(function (result) {
	...
	index.register.factory('apb.getUserData.factory', apbUserDataFactory);
	...
	} index.register.factory('apb.generic.Controller', genericController);

	factoryGenericDetail.$inject = [];
	function factoryGenericDetail () {
		return {
			getDetailInquireVendorDocs: function (id, serie, espec, parcela, estab, callback) {
				return this.TOTVSQuery({id: id, serie: serie, espec: espec, parcela: parcela, estab: estab, method: 'getDetailInquireVendorDocs'}, function (result) {
					if (callback) {
	...
	index.register.factory('apb.generic.detail.factory', factoryGenericDetail);
	...
	function factoryMessage($totvsresource) {
		...
		var factory = $totvsresource.REST('/dts/datasul-rest/resources/api/fch/fchapb/userpreference/messages/:page', { page: '@page' }, {});
	...
	index.register.factory('apb.message.Factory', factoryMessage);
