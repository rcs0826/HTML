...
!(function () {
    ...
    function a(e, r) {
        return e
            .replace(/\s{1,}/g, " ")
            .replace(/ AND /gi, "~::~" + r + r + "AND ")
            .replace(/ BETWEEN /gi, "~::~" + r + "BETWEEN ")
            .replace(/ CASE /gi, "~::~" + r + "CASE ")
            .replace(/ ELSE /gi, "~::~" + r + "ELSE ")
            .replace(/ END /gi, "~::~" + r + "END ")
    ...
    (r.prototype.xml = function (r, c) {
        ...
        for (n = 0; n < s; n++)
            a[n].search(/<!/) > -1
                ? ((i += t[l] + a[n]), (p = !0), (a[n].search(/-->/) > -1 || a[n].search(/\]>/) > -1 || a[n].search(/!DOCTYPE/) > -1) && (p = !1))
                : a[n].search(/-->/) > -1 || a[n].search(/\]>/) > -1
                ? ((i += a[n]), (p = !1))
                : /^<\w/.exec(a[n - 1]) && /^<\/\w/.exec(a[n]) && /^<[\w:\-\.\,]+/.exec(a[n - 1]) == /^<\/[\w:\-\.\,]+/.exec(a[n])[0].replace("/", "")
                ? ((i += a[n]), p || l--)
                ...
                : a[n].search(/\/>/) > -1
                ? (i = i += p ? a[n] : t[l] + a[n])
                : a[n].search(/<\?/) > -1 || a[n].search(/xmlns\:/) > -1 || a[n].search(/xmlns\=/) > -1
                ? (i += t[l] + a[n])
                : (i += a[n]);
        return "\n" == i[0] ? i.slice(1) : i;
    }),
        ...
        (r.prototype.css = function (r, c) {
            var a = r
                ...
                i = 0,
                n = c ? e(c) : this.shift;
            for (i = 0; i < s; i++) /\{/.exec(a[i]) ? (l += n[p++] + a[i]) : /\}/.exec(a[i]) ? (l += n[--p] + a[i]) : (/\*\\/.exec(a[i]), (l += n[p] + a[i]));
            return l.replace(/^\n{1,}/, "");
        }),
        (r.prototype.sql = function (r, s) {
            var p = r
                ...
                N = 0,
                o = s ? e(s) : this.shift;
            for (N = 0; N < l; N++) i = N % 2 ? i.concat(p[N]) : i.concat(a(p[N], t));
